...
export async function CreateCustomer(req, res) {
  try {
    ...
    if (!/^[\p{L}\s]+$/u.test(data.FullName)) {
      res.send(
        "Họ tên khách hàng chỉ bao gồm chữ cái, không được chứa chữ số và kí tự đặc biệt"
      );
      return;
    }
    ...
    if (
      parseInt(
        (
          await db(
            `Select COUNT(*) as 'Count' from Customer where PersonalId = '${data.PersonalId}'`
          )
        ).recordset[0].Count
      ) > 0
    ) {
    ...
    if (data.Phone) {
      if (!/^\d{10,11}$/.test(data.Phone)) {
        res.send(
          "Số điện thoại của khách hàng chỉ chứa chữ số và có độ dài tối thiểu là 10 kí tự và tối đa 11 kí tự"
        );
        return;
      }
      if (
        parseInt(
          (
            await db(
              `Select COUNT(*) as 'Count' from Customer where Phone = '${data.Phone}'`
            )
          ).recordset[0].Count
        ) > 0
      ) {
...
export async function ReadCustomerByName(req, res) {
  try {
    ...
    const result = await db(
      `Select * from Customer where FullName LIKE N'%${customerName}%'`
    );
    res.send(result.recordset);
  } catch (err) {
    console.log(err);
...
export async function ReadCurrentCustomersInHotel(req, res) {
  try {
    const result =
      await db(`Select Customer.CustomerId, FullName, PersonalId, Phone, Type, RoomId, CheckInDate, ExpectedCHeckOutDate from Customer
...
export async function ReadCustomerByType(req, res) {
  try {
    ...
    const result = await db(
      `Select * from Customer where Type = ${customerType}`
    );
    res.send(result.recordset);
  } catch (err) {
    console.log(err);
...
export async function DeleteCustomer(req, res) {
  try {
    ...
    select COUNT(*) as 'Count' from Customer where Customer.CustomerId = ${customerId}`)
    ).recordsets;
    var counter = 0;
    for (let table of countResult) counter += parseInt(table[0].Count);
    if (counter > 1) {
      res.send(
        "Không thể xóa khách hàng vì hệ thống vẫn đang sử dụng dữ liệu của khách hàng này"
      );
      return;
    } else if (counter == 0) {
...
export async function UpdateCustomer(req, res) {
  try {
    ...
    const oldData = (
      await db(
        `Select FullName, PersonalId, Phone, Address, Type from Customer where CustomerId = ${customerId}`
      )
    ).recordset[0];
    if (oldData) {
      ...
      if (newData.PersonalId) {
        if (
          parseInt(
            (
              await db(
                `Select COUNT(*) as 'Count' from Customer where PersonalId = '${newData.PersonalId}'`
              )
            ).recordset[0].Count
          ) > 0
        ) {
      ...
      if (newData.Phone) {
        ...
        if (
          parseInt(
            (
              await db(
                `Select COUNT(*) as 'Count' from Customer where Phone = '${newData.Phone}'`
              )
            ).recordset[0].Count
          ) > 0
        ) {
